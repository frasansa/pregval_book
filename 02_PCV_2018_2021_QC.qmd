---
title: "Quality Check: Processing PCV. Empadronados 2018-2021"
---
...
```{r}
# select the input-----------------------------------------------------------
table_name <- "pcv_2018_2021_clean.csv"
...
In @fig-ccompleteness is shown the percentage of non-missing values for each variable. Non-mandatory variables are shown at the bottom of the figure.
...
In @fig-cvisityear, @fig-cvisitmonth, and @fig-cvisitday are presented the frequencies of years, months, and days of the visits respectively.

<br>

```{r}
#| label: fig-cvisityear
#| fig-cap: "Visit year"
...
The variable *serv_at_desc* is missing in **`r miss_serv_at_desc$missing_obs` observations**, so it is `r colorize(miss_serv_at_desc$print_missing, miss_serv_at_desc$color_missing)` complete. @tbl-visit shows all the services used in the primary care visits arranged by alphabetic order. @fig-cserv shows the count of the utilization of each visit service. Finally, @fig-cservyear shows the count of visits for the 10 most used services per year.
...
The variable *diag_cod* is missing in **`r miss_diag_cod$missing_obs` observations**, so it is `r colorize(miss_diag_cod$print_missing, miss_diag_cod$color_missing)` complete. @fig-ccode shows the most employed diagnoses codes. Finally, @fig-ccodeyear shows the count of the 10 most employed codes per year.
...
The variable *tipo_codigo* is missing in **`r miss_tipo_codigo$missing_obs` observations**, so it is `r colorize(miss_tipo_codigo$print_missing, miss_tipo_codigo$color_missing)` complete. @fig-ctipo shows the count of the utilization of each visit service. Finally, @fig-ctipoyear shows the count of visits for the 10 most used services per year.
