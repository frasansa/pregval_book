---
title: "Quality Check: Processing PCV. Empadronados 2009-2012"
---
...
```{r}
# select the input-----------------------------------------------------------
table_name <- "pcv_2009_2012_clean.csv"
...
In @fig-acompleteness is shown the percentage of non-missing values for each variable. Non-mandatory variables are shown at the bottom of the figure.
...
In @fig-avisityear, @fig-avisitmonth, and @fig-avisitday are presented the frequencies of years, months, and days of the visits respectively.

<br>

```{r}
#| label: fig-avisityear
#| fig-cap: "Visit year"
...
The variable *serv_at_desc* is missing in **`r miss_serv_at_desc$missing_obs` observations**, so it is `r colorize(miss_serv_at_desc$print_missing, miss_serv_at_desc$color_missing)` complete. @tbl-visit shows all the services used in the primary care visits arranged by alphabetic order. @fig-aserv shows the count of the utilization of each visit service. Finally, @fig-aservyear shows the count of visits for the 10 most used services per year.
...
The variable *diag_cod* is missing in **`r miss_diag_cod$missing_obs` observations**, so it is `r colorize(miss_diag_cod$print_missing, miss_diag_cod$color_missing)` complete. @fig-acode shows the most employed diagnoses codes. Finally, @fig-acodeyear shows the count of the 10 most employed codes per year.
...
The variable *tipo_codigo* is missing in **`r miss_tipo_codigo$missing_obs` observations**, so it is `r colorize(miss_tipo_codigo$print_missing, miss_tipo_codigo$color_missing)` complete. @fig-atipo shows the count of the utilization of each visit service. Finally, @fig-atipoyear shows the count of visits for the 10 most used services per year.
